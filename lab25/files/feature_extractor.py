...
def window(x, frame_size, frame_shift, sampling_rate, taper):
	frame_size = int(frame_size * sampling_rate)
	frame_shift = int(frame_shift * sampling_rate)
	n_frames = int(np.ceil((len(x) - frame_size) / frame_shift)) + 1
...
def dft(frames, frame_shift, sampling_rate):
...
def cepstral_lifter(magnitude_spectrum, n_coefficients):
...
def mfcc(mel_filtered_spectrum, n_coefficients):
...
def standard(x, sampling_rate, frame_size, frame_shift, taper='hamming', alpha=0.97, n_mel_filters=26, n_mfcc=12):
	"""Extract feature vectors consisting of
	(1) energy and 12 MFCCs
	(2) deltas
	(3) delta-deltas
	"""
	frames = window(preemphasis(x, alpha), frame_size, frame_shift, sampling_rate, taper)
	f, t, s = dft(frames, frame_shift, sampling_rate)
...
if __name__ == '__main__':
	...
	frames = window(preemphasis(x, 0.097), 0.025, 0.01, fs, 'hamming')
	f, t, s = dft(frames, 0.01, fs)
	...
	ax[2].pcolormesh(t, f, 10 * np.log10(cepstral_lifter(s, 12)))
	ax[3].pcolormesh(t, np.arange(26), 10 * np.log10(cepstral_lifter(ms, 12)))
	plt.show()
	print(standard(x, fs, 0.025, 0.01).shape)
